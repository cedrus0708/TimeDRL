...
def tensor_to_numpy(value: Any) -> np.ndarray:
    """Convert torch tensors / numpy arrays / lists to numpy arrays."""
...
def load_classification_split_arrays(
    ...
    """
    Load a classification split as arrays by iterating the DataLoader.

    Do not read dataset.x directly, because TimeDRL/OOD split wrappers may hide
    the original x/y attributes while __getitem__ still returns (x, y).
    """
    ...
    for batch in loader:
        if not isinstance(batch, (list, tuple)) or len(batch) < 2:
            raise ValueError(
                "Classification loader must return at least (batch_x, batch_y). "
                f"Got batch type: {type(batch)}"
            )

        batch_x, batch_y = batch[0], batch[1]

        x_chunks.append(tensor_to_numpy(batch_x).astype(np.float32))
        y_chunks.append(tensor_to_numpy(batch_y).reshape(-1).astype(np.int64))

    if not x_chunks:
        raise ValueError(f"No samples found for split='{split}'.")

    x = ensure_nlc(np.concatenate(x_chunks, axis=0))
    y = np.concatenate(y_chunks, axis=0).reshape(-1).astype(np.int64)
...
def main() -> None:
    ...
    # Important:
    # - The target eval set must contain both ID and near-OOD target classes.
    # - The external source split should not inherit HAR ID filters from the
    #   target registry/config, otherwise WISDM may be accidentally filtered.
    target_args.id_classes = list(dict.fromkeys(args.id_classes + args.near_ood_classes))
    target_args.near_ood_classes = []
    target_args.far_ood_classes = []

    source_args.id_classes = None
    source_args.near_ood_classes = []
    source_args.far_ood_classes = []
